...
if __name__ == '__main__':
    ...
    # Inferencing
    with torch.no_grad():
        for i_path in tqdm(paths, desc='Inferencing', dynamic_ncols=True):
            ...
            e_x = torch.FloatTensor(np.array([img_ori / 255]))

            print('e_x shape', e_x.shape, 'for image', i_path)

            e2p = A2P(out_dim=net.fp_size)
            ...
            # Ensure up_mask and down_mask are numpy arrays with correct shape
            if isinstance(up_mask, torch.Tensor):
                up_mask = up_mask.cpu().numpy()
            if isinstance(down_mask, torch.Tensor):
                down_mask = down_mask.cpu().numpy()

            # Check shape and squeeze if needed
            if up_mask.ndim == 3 and up_mask.shape[0] == 1:
                up_mask = up_mask.squeeze(0)
            if down_mask.ndim == 3 and down_mask.shape[0] == 1:
